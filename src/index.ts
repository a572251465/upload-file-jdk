...
export async function computedBreakPointProgressHandler(
    ...
    if (isNotEmpty(res!.data)) {
        ...
        // 更新 文件进度状态
        const realProgress = computedInnerProgressHandler(uniqueCode, step * length);
        // 断点续传状态
        emitUploadingProgressState(
...
export async function splitFileUploadingHandler(
    ...
    // 步长, 保留一个小数点
    const step = computedStepHandler(chunks.length);
    ...
    for (; idx < chunks.length && isCanNextExecute(uniqueCode);) {
        ...
        if (res.data) {
            ...
            emitUploadingProgressState(
                UploadProgressState.Uploading,
                uniqueCode,
                computedInnerProgressHandler(uniqueCode, step),
            );
            idx += 1;
        } else {
            // 判断 是否重试失败
...
uploadHandler.dynamicFileSizeLimitRules = function (
    ...
    // 设置规则
    calculateUploaderConfig.current!.fileSizeLimitRules = limitRules;
};
